...
--[[Changelog
    ...
    unreleased (ctrlfrk): messed aroudn with a bunch of stuff and added in an army supply display and a tech indicator
]]
--------------------------------------------------------------------------------
-- Config
--------------------------------------------------------------------------------
...
function UpdatePlayerResources()
    -- dsx put https://springrts.com/wiki/Lua_SyncedRead#GetTeamUnits here
    -- use global https://springrts.com/wiki/Lua_UnitDefs to get costs
    local energy, energyStorage, metal, metalStorage = 0, 1, 0, 1
    local energyIncome, metalIncome
    local supplyTotal, supplyArmy, supplyEconomy, supplyBuildpower
    local dsxTechTier, dsxTechType, dsxTechProgress
    ...
    for playerID, _ in pairs(player) do
        if player[playerID].name and not player[playerID].spec and player[playerID].team then
            if aliveAllyTeams[player[playerID].allyteam] ~= nil and (mySpecStatus or myAllyTeamID == player[playerID].allyteam) then
                ...
                -- supply calcs
                supplyTotal, supplyArmy, supplyEconomy, supplyBuildpower = 0, 0, 0, 0
                dsxTechTier = 0
                dsxTechType = '???'
                dsxTechProgress = 0
                ...
                --[[
                    ...
                    coramsub: amphib
                    corasy: t2 sea
                    corfhp: floating hover
                    corgantuw: t3 underwater
                    corplat: seaplanes
                    corsy: t1 sea
                --]]

                -- Spring.Echo('UnitDef: -------' )
                for _, unitID in pairs(Spring.GetTeamUnits(player[playerID].team)) do 
                    -- dsxTotalSupply = dsxTotalSupply + 1
                    local unitDefID = Spring.GetUnitDefID(unitID)
                    local health, _, _, _, buildprogress = Spring.GetUnitHealth(unitID)      
                    local newTechTier = 0
                    local newTechType = "???"

                    local newTechProgress = 0

                    -- calc tech level
                    if unitDefID then
                        local def = UnitDefs[unitDefID]
                        if def.name == "armshltx" or def.name == "armshltxuw" then newTechType, newTechTier = "exp", 3 end
                        if def.name == "corgant" or def.name == "corgantuw" then newTechType, newTechTier = "exp", 3 end

                        if def.name == "armalab" or def.name == "coralab" then newTechType, newTechTier = "bot", 2 end
                        if def.name == "armavp" or def.name == "coravp" then newTechType, newTechTier = "veh", 2 end
                        if def.name == "armasy" or def.name == "corasy" then newTechType, newTechTier = "sea", 2 end
                        if def.name == "armaap" or def.name == "coraap" then newTechType, newTechTier = "air", 2 end

                        if def.name == "armlab" or def.name == "corlab" then newTechType, newTechTier = "bot", 1 end
                        if def.name == "armvp" or def.name == "corvp" then newTechType, newTechTier = "veh", 1 end
                        if def.name == "armsy" or def.name == "corsy" then newTechType, newTechTier = "sea", 1 end
                        if def.name == "armhp" or def.name == "corhp" then newTechType, newTechTier = "hov", 1 end
                        if def.name == "armfhp" or def.name == "corfhp" then newTechType, newTechTier = "hov", 1 end
                        if def.name == "armamsub" or def.name == "coramsub" then newTechType, newTechTier = "amp", 1 end
                        if def.name == "armap" or def.name == "corap" then newTechType, newTechTier = "air", 1 end
                        if def.name == "armplat" or def.name == "corplat" then newTechType, newTechTier = "air", 1 end

                        if newTechTier > 0 then
                            if (newTechTier > dsxTechTier) or
                                (newTechTier == dsxTechTier and dsxpriority[newTechType] > dsxpriority[dsxTechType]) or
                                (newTechTier == dsxTechTier and dsxpriority[newTechType] == dsxpriority[dsxTechType] and buildprogress > dsxTechProgress) 
                            then
                                dsxTechTier = newTechTier
                                dsxTechType = newTechType
                                dsxTechProgress = buildprogress
                            end
                        end
                    end
                    ...
                    if
                        unitDefID and
                        buildprogress == 1.0 and
                        not UnitDefs[unitDefID].modCategories.commander
                    then
                        local def = UnitDefs[unitDefID]

                        supplyTotal = supplyTotal + def.metalCost

                        if def.buildSpeed > 0 and not def.isBuilding then
                            supplyBuildpower = supplyBuildpower + def.metalCost
                        end

                        -- if def.isBuilding and (def.windGenerator > 0 or def.tidalGenerator > 0 or def.extractsMetal > 0) then
                        if def.isBuilding and (
                            def.customParams.solar or
                            (
                                def.energyMake > 19 and (
                                    not def.energyUpkeep or
                                    def.energyUpkeep < 10
                                ) or
                                def.energyUpkeep < -19 or def.windGenerator > 0 or def.tidalGenerator > 0
                            )
                        ) then
                            supplyEconomy = supplyEconomy + def.metalCost
                        end

                        if #def.weapons > 0 then
                            supplyArmy = supplyArmy + def.metalCost
                        end
                    end
                end
                ...
                player[playerID].dsxTotalSupply = "army: " .. supplyArmy .. ", economy: " .. supplyEconomy .. ", builders: " .. supplyBuildpower
                player[playerID].dsxArmySupply = supplyArmy
                player[playerID].dsxTechTier = dsxTechTier
                player[playerID].dsxTechType = dsxTechType
                player[playerID].dsxTechProgress = dsxTechProgress
            end
        end
    end
end
...
function DrawPlayer(playerID, leader, vOffset, mouseX, mouseY)
    ...
    if spec == false then
        ...
        if m_dsxsupply.active then
            -- DrawSupply(player[playerID].dsxTotalSupply, posY, dark)
            local col = "\255\192\000\000"
            local tech = player[playerID].dsxTechType
            -- if (player[playerID].dsxTechTier > 1) then tech = 'T ' .. player[playerID].dsxTechTier end
            if (player[playerID].dsxTechTier == 1) then col = "\255\255\127\000" end
            if (player[playerID].dsxTechTier == 2) then col = "\255\255\255\000" end
            if (player[playerID].dsxTechTier == 3) then col = "\255\000\255\000" end
            if (player[playerID].dsxTechProgress > 0.0 and player[playerID].dsxTechProgress < 1.0) then tech = math.floor(player[playerID].dsxTechProgress * 100) .. '%' end

            DrawSupply(ToSI(player[playerID].dsxArmySupply, false)..' '..col..tech, posY, dark)
            -- if tipY then
            --     DrawSupply(player[playerID].dsxTotalSupply, posY, dark)
            -- end
        end
...
function DrawSupply(totalSupply, posY, dark)
    font:Begin()
    font:Print(totalSupply, m_dsxsupply.posX + widgetPosX + m_dsxsupply.width - 2, posY + 5.3, 9.5, "or")
    font:End()
end
